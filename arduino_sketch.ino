...
#define RightB    6                                           // Right Button Pin
#define MouseB    7                                           // Mouse Enable Button Pin

MPU6050 mpu6050(Wire);
int X,Y,Z;                                                    // Data Variables for mouse co-ordinates
int OX,OY,OZ;                                                 // Angle Variables for calucating gyroscope zero error
SoftwareSerial bluetooth(2, 3);                               // SoftwareSerial bluetooth(RX_pin, TX_pin);

void setup() {
  bluetooth.begin(9600);                                    // Bluetooth baudrate matching Serial's baud rate
  Wire.begin();                                               // Initialise I2C communication
  mpu6050.begin();                                            // Initialise Gyro communication
  bluetooth.println("STRTM");                                 // Identifier "STARTM" for start mouse
...
void loop() {
  ...
  if(digitalRead(MouseB) == HIGH){                                     // Checks if Mouse Enabled Button is pushed
    ...
    bluetooth.println("DATAL,"+String(X)+','+String(Y)+','+String(Z)); // Sends corrected gyro data to the Serial Port with the identifier "DATAL"
  }
  if(digitalRead(LeftB) == HIGH){                                      // Checks if Left Mouse Button is pushed
    delay(100);                                                        // Debounce delay
    bluetooth.println("DATAB,L");                                      // Sends "L" stating that the left button is pressed with the identifier "DATAB"
  }
  if(digitalRead(RightB) == HIGH){                                     // Checks if Right Enabled Button is pushed
    delay(100);                                                        // Debounce delay
    bluetooth.println("DATAB,R");                                      // Sends "R" stating that the right button is pressed with the identifier "DATAB"
  }
}